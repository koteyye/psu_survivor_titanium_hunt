...
export function createGameUI(scene, options = {}) {
    ...
    scene.scoreText = scene.add.text(100, 40, score.toString(), {
        fontFamily: 'Orbitron',
        fontSize: '36px',
        fill: '#fff',
        stroke: '#000',
    ...
    if (settings.showHealthBar) {
        const health = scene.registry.get('health') || 100;
...
export function updateGameUI(scene) {
    ...
    if (scene.healthBar) {
        const health = scene.registry.get('health') || 100;
        scene.healthBar.setValue(health);
    }
    ...
    if (scene.moneyBar) {
        ...
        if (money !== undefined) {
            scene.moneyBar.setValue(money);
        }
    }
    
    // Обновляем шкалу ярости, если она есть (для будущего функционала)
    if (scene.rageBar) {
        const rage = scene.registry.get('rage');
        if (rage !== undefined) {
            scene.rageBar.setValue(rage);
        }
    }
}
