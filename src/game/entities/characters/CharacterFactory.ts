import { ConfigManager, EventManager } from '../../../systems/managers';
...
export class CharacterFactory {
  ...
  public static createCharacter(
    ...
    // Сохраняем персонажа в Registry сцены
    scene.registry.set('gameCharacter', character);
    scene.registry.set('playerSprite', character.getSprite());
    
    // Уведомляем о создании персонажа через EventManager сцены
    scene.events.emit('characterCreated', { characterId, character });
    
    // Логируем создание персонажа
    console.log(`✅ Character ${characterId} registered in scene successfully`);
